...
void test01() {
    ...
    for (int i = 0; i < 32; i++) printf("i = %d\tfrom thread No. %d\n", i, omp_get_thread_num());// 返回线程编号
    #pragma omp parallel for
    for (int i = 0; i < 32; i++) cout << "i = " << i << "\tfrom thread No. " << omp_get_thread_num() << endl;
}
